...
public class IndexableException extends Exception implements Indexable {
	...
	public IndexableException(Indexable indexable, String message) {
		super("On line " + indexable.getLineNumber() + " column " + indexable.getColumnNumber() + ": " + message);
		this.inner = indexable;
		this.message = message;
		this.fullText = "On line " + indexable.getLineNumber() + " column " + indexable.getColumnNumber() + ": "
				+ message;
	}
	...
	public String getLineAnnotation() {
		...
		for (int i = inner.getColumnNumber() - 1; i < inner.getColumnNumber() + inner.getText().length() - 1; i++) {
			lineAnnotation.append('^');
		}
