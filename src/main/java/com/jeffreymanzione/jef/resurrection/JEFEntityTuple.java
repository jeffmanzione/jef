...
public abstract class JEFEntityTuple extends JEFEntity<Integer> {
	...
	private Map<Integer, Object> indicesToObjs = new HashMap<Integer, Object>();
	private Map<Integer, Class<?>> indicesToClasses = new HashMap<Integer, Class<?>>();
	...
	public boolean set(Integer key, Object val) throws CouldNotUpdateEntityException {
		if (key < 0 || key > size()) {
			...
			for (Field field : this.getClass().getDeclaredFields()) {
				if (field.isAnnotationPresent(JEFTuple.class) && field.getAnnotation(JEFTuple.class).value() == key) {
			...
			indicesToObjs.put(key, val);
			indicesToClasses.put(key, val.getClass());
			return true;
	...
	public Object get(Integer key) throws CouldNotUpdateEntityException {
		if (key < 0 || key > size()) {
			...
			if (indicesToObjs.containsKey(key)) {
				return indicesToObjs.get(key);
			} else {
	...
	public Class<?> getType(Integer key) throws CouldNotUpdateEntityException {
		if (key < 0 || key > size()) {
			...
			if (indicesToObjs.containsKey(key)) {
				return indicesToClasses.get(key);
			} else {
				throw new CouldNotUpdateEntityException(
						"Could not find key! This means the class was constructed "
								+ "incorrectly. Either no field is labeled with this index or the size() method is wrong. index="
