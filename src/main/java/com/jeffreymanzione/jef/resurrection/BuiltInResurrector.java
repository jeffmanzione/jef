...
public final class BuiltInResurrector {
	...
	static {
		...
		addTransformer(new Transformer<Color>(ValueType.TUPLE, ColorDefinition.instance(), Color.class) {
			...
			public Color protectedTransformResurrection(Value<?> input) {
				...
				Color color = new Color((Integer) tupleValue.get(1).getValue(), (Integer) tupleValue.get(2).getValue(),
						(Integer) tupleValue.get(3).getValue(), (Integer) tupleValue.get(0).getValue());
				return color;
			}
	...
	public static JEFEntity<?> transformObject(Object obj) throws CouldNotTranformValueException {
		if (classesToTransformers.containsKey(obj.getClass())) {
			return ((Transformer<Object>) classesToTransformers.get(obj.getClass())).transformToValue(obj);
		} else {
			throw new CouldNotTranformValueException("No transformer could be found for specified object=" + obj
	...
	public static abstract class Transformer<T> {
		...
		protected abstract T protectedTransformResurrection(Value<?> input) throws CouldNotTranformValueException;
		...
		public final T transformToClass(Value<?> input) throws CouldNotTranformValueException {
			if (correctType(input, type) && input.hasEntityID() && input.getEntityID().equals(id)) {
				...
				throw new CouldNotTranformValueException(
						"Input did not have the proper type during transformation of value to object. Value =" + input
								+ " of type=" + input.getType() + " but expected type=" + type);
			}
		}
		...
		public final JEFEntity<?> transformToValue(T input) throws CouldNotTranformValueException {
			if (input.getClass().equals(externalClass)) {
				...
				throw new CouldNotTranformValueException("No way to transform this class was specified. Type="
	...
	private static boolean correctType(Value<?> val, ValueType type) {
		if (val instanceof TupleValue && type == ValueType.TUPLE || val instanceof MapValue && type == ValueType.MAP
				|| val instanceof ListValue && type == ValueType.LIST || val instanceof EnumValue
				&& type == ValueType.ENUM) {
			return true;
		} else {
			return false;
